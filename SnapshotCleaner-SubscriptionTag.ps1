<#
    .SYNOPSIS
    Retrieve all snapshots and delete them if they are older than 14 days and have a tag "Environment = Development" on the subscription
    ...
    As a safety blanket it will only delete the snapshot if there is a tag "Environment = Development" on the subscription
...
#Retrieve all Azure Subscriptions with the tag "Environment = Development"
$Subscriptions = Get-AzSubscription | Where-Object {$_.Tags.Environment -eq "Development"}
...
foreach ($sub in $Subscriptions) {
    ...
    foreach ($RG in $RGs) {
        ...
        foreach ($Snapshot in $Snapshots) {
            ...
            elseif ($Snapshot.TimeCreated -lt (Get-Date).AddDays(-14) {
